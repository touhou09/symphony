defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server forwards model overrides in turn/start payload" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-turn-model-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      codex_binary = Path.join(test_root, "fake-codex")
      trace_file = Path.join(test_root, "codex-turn-model.trace")
      previous_trace = System.get_env("SYMP_TEST_CODEx_TRACE")
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${SYMP_TEST_CODEx_TRACE:-/tmp/codex-turn-model.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' \"$line\" >> \"$trace_file\"
      ...
      issue = %Issue{
        id: "issue-turn-model-override",
        identifier: "MT-1004",
        title: "Validate model override",
        description: "Ensure model values are forwarded to codex turn/start payloads.",
        state: "In Progress",
      ...
      assert {:ok, _result} =
               AppServer.run(workspace, "Validate model override", issue, model: "gpt-5.5")
      ...
      assert Enum.any?(lines, fn line ->
               if String.starts_with?(line, "JSON:") do
                 payload = String.trim_leading(line, "JSON:") |> Jason.decode!()

                 payload["method"] == "turn/start" && payload["params"]["model"] == "gpt-5.5"
               else
                 false
               end
             end)
